...
def time_delta(t1, t2):
    # Since the given time is string 
    # Coverting the str type into time object
    # using strptime()
    time_format="%a %d %b %Y %H:%M:%S %z" 
    t1=datetime.strptime(t1,time_format)
    t2=datetime.strptime(t2,time_format)
    return (str(int(abs(t1-t2).total_seconds()))) # Total_seconds() rertuns the no. fo seconds per hour or minutes
